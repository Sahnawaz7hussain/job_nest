...
const postNewJobRequest = () => {
  return { type: types.POST_NEWJOB_REQUEST };
};
const postNewJobSuccess = (payload) => {
  return { type: types.POST_NEWJOB_SUCCESS, payload };
};
const postNewJobFailure = () => {
  return { type: types.POST_NEWJOB_FAILURE };
};
const getJobsData = (dispatch) => {
  dispatch(getJobsDataRequest());

  axios
    .get(`https://fake-api-bytv.onrender.com/api/recommendedjobs`)
    .then((res) => {
...
const getSingleJobDetails = (id) => (dispatch) => {
  ...
  axios
    .get(`https://fake-api-bytv.onrender.com/api/recommendedjobs?id=${id}`)
    .then((res) => {
...
// POST NEW JOB ACTION FUNCTION.
const postNewJobActionFn = (data) => (dispatch) => {
  dispatch(postNewJobRequest());
  return axios
    .post("https://fake-api-bytv.onrender.com/api/recommendedjobs", data)
    .then((res) => {
      return dispatch(postNewJobSuccess(res.data));
    })
    .catch((err) => {
      dispatch(postNewJobFailure(err));
    });
};
...
export { getJobsData, getSingleJobDetails, postNewJobActionFn };
